...
export default function FilterBar({ filters, onFilterChange }: FilterBarProps) {
  ...
  useEffect(() => {
    if (
      ...
      regions[localFilters.region.sido as keyof typeof regions]
    ) {
      setAvailableGugun(
        regions[localFilters.region.sido as keyof typeof regions],
      );
    ...
    <div className="space-y-4 w-full max-w-full">
      ...
      <div className="space-y-2 lg:hidden">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            className="h-11 px-4 bg-[#1B59FA] hover:bg-blue-700 text-white flex items-center gap-2 flex-shrink-0"
          >
      ...
      <div className="lg:hidden">
        ...
        >
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <Filter className="h-5 w-5 text-[#1B59FA] flex-shrink-0" />
            <div className="text-left min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                {getActiveFilterCount() > 0 && (
                  <Badge
                    variant="secondary"
                    className="bg-[#1B59FA] text-white text-xs px-2 py-0.5 flex-shrink-0"
                  >
      ...
      <div
        className={cn(
          ...
          isMobileFilterOpen
            ? "block opacity-100 max-h-[2000px] overflow-visible" // 모바일에서 열렸을 때
            : "lg:block max-h-0 opacity-0 lg:opacity-100 lg:max-h-none overflow-hidden", // 모바일에서 닫혔을 때
        )}
      >
        {/* 모바일에서 열렸을 때 상단 여백 */}
        ...
        <div className="flex flex-col space-y-4 lg:grid lg:grid-cols-5 lg:gap-4 lg:space-y-0 w-full">
          {/* 날짜 범위 필터 */}
              ...
              onChange={(value) => {
                if (value && typeof value === "object" && "from" in value) {
                  ...
                  setLocalFilters({
                    ...localFilters,
                    dateRange: {
                      from: range.from || localFilters.dateRange.from,
                      to: range.to || localFilters.dateRange.to,
                    },
                  });
                }
              }}
          ...
          <div className="space-y-2 w-full min-w-0">
            ...
            <Select
              value={localFilters.selectedCategory || "전체"}
            ...
            >
              ...
              <SelectContent className="max-h-[300px] z-50 w-[var(--radix-select-trigger-width)] max-w-[calc(100vw-2rem)]">
                <SelectItem
                  value="전체"
                  className="text-sm py-2 px-3 cursor-pointer hover:bg-gray-100 focus:bg-gray-100"
                >
          ...
          {/* 지역 필터 - 모바일에서는 세로 배치, 데스크톱에서는 가로 배치 */}
          <div className="space-y-2 w-full min-w-0">
            <Label className="text-sm font-medium text-gray-700">지역</Label>
            <div className="flex flex-col gap-2 lg:flex-row lg:gap-2">
              <Select
                value={localFilters.region.sido || "전체"}
                onValueChange={handleSidoChange}
              >
                <SelectTrigger className="w-full lg:flex-1 h-11 text-sm flex items-center justify-between px-3 py-2 border rounded-md bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#1B59FA] focus:border-[#1B59FA] transition-colors">
                  <SelectValue
                    placeholder="시/도"
                    className="text-left truncate"
                  />
                </SelectTrigger>
                <SelectContent className="z-50 w-[var(--radix-select-trigger-width)] max-w-[calc(100vw-2rem)]">
                  <SelectItem
                    value="전체"
                    className="text-sm py-2 px-3 cursor-pointer hover:bg-gray-100 focus:bg-gray-100"
                  >
              ...
              >
                <SelectTrigger
                  className={cn(
                    "w-full lg:flex-1 h-11 text-sm flex items-center justify-between px-3 py-2 border rounded-md bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#1B59FA] focus:border-[#1B59FA] transition-colors",
                    !localFilters.region.sido &&
          ...
          <div className="space-y-2 w-full">
            ...
            <div className="flex items-center gap-3 min-h-[44px] pr-2 w-full">
              <Switch
              ...
              <Label
                ...
                className="cursor-pointer text-sm flex-1 break-words"
              >
                연락처 있음만 보기
              </Label>
            </div>
          ...
          {/* 키워드 검색 - PC에서만 보이는 영역 */}
          <div className="space-y-2 hidden lg:block w-full min-w-0">
            <Label className="text-sm font-medium text-gray-700">
              병원명 검색
            </Label>
            <Input
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 mt-6 w-full">
          <Button
            ...
            className="w-full sm:w-auto h-11 text-sm font-medium flex-shrink-0"
          >
            초기화
          </Button>
          <Button
            className="w-full sm:w-auto h-11 text-sm font-medium bg-[#1B59FA] hover:bg-blue-700 flex-shrink-0"
            onClick={() => {
              applyFilters();
